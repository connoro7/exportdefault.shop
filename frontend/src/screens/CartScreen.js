...
const CartScreen = ({ match, location, history }) => {
  ...
  const checkoutHandler = () => {
    ...
    // If YES signed in, redirect to shipping
    ...
    <Row>
      ...
      <Col md={4}>
        <Card>
          <ListGroup variant='flush'>
            <ListGroup.Item>
              <h2>Subtotal ({cartItems.reduce((acc, item) => acc + item.quantity, 0)}) Items</h2>${cartItems.reduce((acc, item) => acc + item.quantity * item.price, 0).toFixed(2)}
            </ListGroup.Item>
            <ListGroup.Item>
